  ...
  methods: {
    ...
    subConfirm(index){
      ...
      this.MasterList.map(item=>{
        console.log(item.bindName,"1111111")
        console.log(this.list[this.active].RealName,"2222222")
        console.log(item.bindName !== this.list[this.active].RealName)
        ...
        if(num==this.MasterList.length &&　num!=0){
          ...
          this.MasterList[index].showType = false
          this.active=0
        }else if(num<this.MasterList.length &&　num!=0){
          wx.showToast({
              title:"此订单中已存在该师傅！",
              icon:"none",
              duration:1500
      ...
      if(toLogin(this.curPage)){
        post('ExecuteOrder/ExecuteArrangeMaster',{
          ...
          if(res.code==0){
            ...
            setTimeout(() => {
              wx.redirectTo({
                url: "/pages/perform/myOrder/main" //我的订单
              });
            }, 1000);
          }
        })
