  ...
  <div class="page">
    ...
    <div class="orderInfo bg_fff mb10" v-if="InstallList.length>0">
      ...
      <div class="orderInfo__bd" v-for="(item,sindex) in InstallList" :key="sindex">
        <div class="item" v-if="item.IsMain==1">
          主负责师傅：{{item.MasterName}}
          <span class="btnCopy" @tap="copy(detail.orderNo)">重新指派</span>
        </div>
        <div class="item" v-if="item.IsMain==0">
          其他师傅：{{item.MasterName}}
          <span class="btnCopy" @tap="copy(detail.orderNo)">重新指派</span>
        </div>
      </div>
    </div>
